-- Update the AI usage function to handle the new free models
...
BEGIN
  ...
  IF p_is_registered THEN
    ...
    CASE p_ai_model
      -- New free models with higher limits for registered users
      ...
      -- Legacy free models (high limits for registered users)
      WHEN 'llama-3.1-8b' THEN v_daily_limit := 200;
  ...
  ELSE
    ...
    CASE p_ai_model
      -- New free models with basic limits for non-registered users
      WHEN 'gpt-3.5-turbo-free' THEN v_daily_limit := 100;
      WHEN 'claude-3-haiku-free' THEN v_daily_limit := 80;
      WHEN 'gemini-1.5-flash-free' THEN v_daily_limit := 90;
      WHEN 'deepseek-v3-free' THEN v_daily_limit := 60;
